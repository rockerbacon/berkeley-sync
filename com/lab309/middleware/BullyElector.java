...
public class BullyElector {
	...
	private static final byte[] newLeaderMsg = {(byte)0x5E};
	...
	public boolean becomesLeaderUponElection () {
		...
		new Thread (new Runnable () { @Override public void run () {
			...
			try {
				...
				candidature.getBuffer().put(BullyElector.electionMsg);
				candidature.getBuffer().putInt(BullyElector.this.id);
				...
				synchronized (BullyElector.this.electionLock) {
					/*
					...
					*/
					...
					BullyElector.this.lastLeaderActivity = BullyElector.this.clock.getTimeMillis();		
					//notify processes of new leader
					candidature.getBuffer().rewind();
					candidature.getBuffer().put(BullyElector.newLeaderMsg);
					for (InetAddress addr : broadcast) {
						UDPClient c = new UDPClient(BullyElector.this.port, addr, null);
						c.send(candidature);
						c.close();
				...
				while (BullyElector.this.monitoringLeader) {
					synchronized (BullyElector.this.electionLock) {
						if (BullyElector.this.clock.getTimeMillis()-BullyElector.this.lastLeaderActivity > BullyElector.this.maxInactivityInterval) {
							...
							dtg.getBuffer().put(BullyElector.electionMsg);
							dtg.getBuffer().putInt(BullyElector.this.id);
							for (InetAddress addr : broadcast) {
								UDPClient c = new UDPClient(BullyElector.this.port, addr, null);
								c.send(dtg);
								c.close();
